...
static int bbkeys_get(struct device *dev, unsigned bit)
{
	struct bbkeys_info *bb = dev_get_drvdata(dev);
	u32 readval = bbkeys_read(bb, u8pos(bit));
	return readval & u8bit(bit);
}

static void bbkeys_set(struct device *dev, unsigned bit, int val)
{
	struct bbkeys_info *bb = dev_get_drvdata(dev);
	int pos = u8pos(bit);
	unsigned long flag;
...
static int bbkeys_probe(struct platform_device *pdev)
{
	...
	pdata->ops.get = bbkeys_get;
	pdata->ops.set = bbkeys_set;
	pdata->ops.to_irq = bbkeys_to_irq;
