					      ...
					      size_t buffer_size, size_t period_size)
{
	...
	if (ret < 0) {
		snd_printk(KERN_ERR "Cannot allocate buffer, err: %d\n", ret);
		return ret;
	}
	...
	if (!s->dma_dring_store) {
		s->dma_dring_size = 0;
		snd_pxa2xx_i2sound_ngn_free_buffer(s);
		snd_printk(KERN_ERR "Cannot allocate DMA descriptors\n");
		return -ENOMEM;
	}
					       ...
					       unsigned int rate)
{
 ...
 error:	spin_unlock(&card.engine.rate_lock);
	snd_printk(KERN_ERR "Cannot set I2S clock rate %d\n", rate);
	return -EINVAL;
}
...
static int snd_pxa2xx_i2sound_ngn_request_dma_channel(struct snd_pxa2xx_i2sound_stream *s)
{
	...
	if (ret < 0) {
		snd_printk(KERN_ERR "Cannot request DMA channel, err: %d\n", ret);
		return ret;
	}
	s->dma_ch = ret;
...
static int snd_pxa2xx_i2sound_ngn_stop(struct snd_pxa2xx_i2sound_stream *s)
{
	...
	if (timeout == 0) {
		snd_printk(KERN_ERR "Timeout waiting DMA completion on channel %d\n", s->dma_ch);
	}
...
/*
  Flush internal I2S FIFO. PXA27x manual has special clauses that I2S
  controller, once being disabled, enters shutdown phase and stays
  in it until both receive and transmit FIFOs are fully flushed.
  We just hope here that codec will take care of slurping any data from
  transmit FIFO (i.e. codec should not be disabled/powered off until
  I2S controller is ready for that). As for the receive FIFO, we take 
  care of it with this function. Note that flushing FIFO is really good 
  thing to do, because neither I2S reset or clk disable help much with 
  recovering I2S locked in shutdown.

  Also, note that PXA25x manual doesn't have such clauses like PXA27x has, 
  (but they are likely in errata somewhere).
*/
static void flush_pxa_i2s_fifo(void)
{
	int i = 17, tmp;
	
	//snd_printk(KERN_INFO "%s: in SASR0=%x\n", __FUNCTION__, SASR0);
	while (SASR0 & SASR0_I2SOFF && i > 0) {
		tmp = SADR;
		i--;
	}

	if (i == 0) {
		printk(KERN_ERR "Timeout waiting for I2S shutdown, FIFO flushing didn't help");
	}
	//snd_printk(KERN_INFO "%s: out SASR0=%x\n", __FUNCTION__, SASR0);
}
...
{
	/* Make sure that I2S is in consistent state before 
	   starting to do anything with it. This probably should 
	   be done in snd_pxa2xx_i2sound_i2slink_off() too. */
        flush_pxa_i2s_fifo();
	...
	// Please do not remove this line, it was found to magically
	// resolve issues with pxa27x CPU, ak4641 code, and booting
	// with HaRET. It should be investigate further and real solution
	// found (see CVS log).
	printk("SACR0 = 0x%x, SADIV = 0x%x\n", SACR0, SADIV);
	SACR0 = SACR0_RST;
	if (info_clock_from_pxa) {
...
void snd_pxa2xx_i2sound_i2slink_free(void)
{
	spin_lock(&card.i2slink.lock);
	if (card.i2slink.usage_count == 0) {
		printk(KERN_ERR "pxa2xx-i2sound: Reference counter underflow!\n");
	} else {
		if (--card.i2slink.usage_count == 0 &&
		    !card.i2slink.suspended)
...
int snd_pxa2xx_i2sound_card_activate(struct snd_pxa2xx_i2sound_board *board)
{
	...
	if ((card.acard = snd_card_new(-1, card.board->acard_id, THIS_MODULE, 0)) == NULL) {
		card.acard = NULL;
		ret = -ENOMEM;
		snd_printk(KERN_ERR "Cannot allocate snd_card object for %s\n", card.name);
	...
	if ((ret = board->add_mixer_controls(card.acard)) < 0) {
		snd_printk(KERN_ERR "Cannot add mixer controls, err: %d\n", ret);
		goto failed_create_acard;
	}

	/* PCM */
	if ((ret = snd_pxa2xx_i2sound_card_create_pcm()) < 0) {
		snd_printk(KERN_ERR "Cannot create PCM object, err: %d\n", ret);
		goto failed_create_acard;
	}

	/* register the card */
        if ((ret = snd_card_register(card.acard)) < 0) { 
		snd_printk(KERN_ERR "Cannot register sound card, err: %d\n", ret);
		goto failed_create_acard;
	}

	if ((ret = snd_pxa2xx_i2sound_pdev_register()) < 0) {
		snd_printk(KERN_ERR "Cannot register sound platform device, err: %d\n", ret);
